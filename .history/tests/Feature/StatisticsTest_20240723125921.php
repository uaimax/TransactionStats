...
class StatisticsTest extends TestCase
{
    ...
    public function test_get_statistics(): void
    {
        ...
        $response1 = $this->postJson('/api/transactions', ['amount' => 100.10, 'timestamp' => Carbon::now()->subSeconds(10)->toISOString()]);
        Log::info('Resposta da adição de transação 1', ['response' => $response1->getContent()]);

        $response2 = $this->postJson('/api/transactions', ['amount' => 100.75, 'timestamp' => Carbon::now()->subSeconds(20)->toISOString()]);
        Log::info('Resposta da adição de transação 2', ['response' => $response2->getContent()]);

        $response3 = $this->postJson('/api/transactions', ['amount' => 100.20, 'timestamp' => Carbon::now()->subSeconds(30)->toISOString()]);
        Log::info('Resposta da adição de transação 3', ['response' => $response3->getContent()]);

        $response4 = $this->postJson('/api/transactions', ['amount' => 300.25, 'timestamp' => Carbon::now()->subSeconds(40)->toISOString()]);
    ...
    public function test_get_statistics_after_cleanup(): void
    {
        ...
        // Adicionando transações válidas via API
        $this->postJson('/api/transactions', ['amount' => 200, 'timestamp' => Carbon::now()->subSeconds(10)->toISOString()]);
        $this->postJson('/api/transactions', ['amount' => 300, 'timestamp' => Carbon::now()->subSeconds(30)->toISOString()]);
