...
class StatisticsControllerTest extends TestCase
{
    ...
    public function test_get_statistics_after_cleanup(): void
    {
        ...
        Log::info('Log 1: Adicionando uma transação antiga via API');
        ...
        Log::info('Log 2: Adicionando transação válida 1 via API');
        $response1 = $this->postJson('/api/transactions', ['amount' => 200.10, 'timestamp' => Carbon::now()->subSeconds(10)->format('Y-m-d\TH:i:s.v\Z')]);
        Log::info('Log 2: Response 1 status', ['status' => $response1->getStatusCode()]);

        Log::info('Log 3: Adicionando transação válida 2 via API');
        ...
        Log::info('Log 4: Verificando as estatísticas via API');
