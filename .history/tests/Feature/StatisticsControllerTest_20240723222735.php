...
class StatisticsControllerTest extends TestCase
{
    ...
    public function test_cleanup_old_transactions_via_statistics_endpoint(): void
    {
        // Adiciona transações válidas
        $this->postJson('/api/transactions', ['amount' => 100.50, 'timestamp' => Carbon::now()->subSeconds(30)->format('Y-m-d\TH:i:s.v\Z')]);
        $this->postJson('/api/transactions', ['amount' => 200.75, 'timestamp' => Carbon::now()->subSeconds(10)->format('Y-m-d\TH:i:s.v\Z')]);

        // Simula a passagem do tempo para expirar as transações
        sleep(61);

        // Chama o endpoint de estatísticas para acionar a limpeza
        $response = $this->getJson('/api/statistics');
        $response->assertStatus(200)->assertJson([
            'sum' => 0.0,
            'avg' => 0.0,
