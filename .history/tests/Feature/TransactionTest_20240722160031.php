...
class TransactionTest extends TestCase
{
    ...
    public function test_store_transaction_older(): void 
    {
        $response = $this->postJson('/api/transactions', [
            'amount' => 100.50,
            'timestamp' => Carbon::now()->subSeconds(61)->format('Y-m-d\TH:i:s.u\Z'),
        ]);
    ...
    public function test_store_transaction_future_timestamp(): void
    {
        $response = $this->postJson('/api/transactions', [
            'amount' => 100.50,
            'timestamp' => Carbon::now()->addSeconds(61)->format('Y-m-d\TH:i:s.u\Z'),
        ]);

        $response->assertStatus(422);
    }

    // Efetua verificações que retornem o 
