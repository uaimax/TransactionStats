...
class StatisticsHelper 
{
    ...
    public function getStatistics() {
        ...
        // Pegamos o horário atual -1 min.
        $previousMinute = Carbon::now()->subMinutes(1)->timestamp;
        
        // Calcula o índice do minuto dentro da janela de tempo.
        $minuteIndex = intdiv($previousMinute, 60) % $this->timeWindow;

        if(!isset($this->statistics[$minuteIndex])){
            $this->statistics[$minuteIndex] = ['sum'=> 0, 'count' => 0, 'max' => 0, 'min' => 0];
        }
        // Calculamos a média das transações.
        if($this->statistics[$minuteIndex]['count']$avg = $this->statistics[$minuteIndex]['sum'] / $this->statistics[$minuteIndex]['count'];
