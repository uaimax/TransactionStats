...
class StatisticsHelper 
{
    ...
    public function cleanUpOldTransactions(&$data)
    {
        ...
        // Se ainda houver transações, recalculamos os valores máximo e mínimo.
        if ($data['count'] > 0) {
            $this->recalculateMinMax($data);
        } else {
            // Se não houver transações, resetamos os valores máximo e mínimo.
    ...
    {
        // Inicializamos os valores máximo e mínimo.
        $data['max'] = PHP_FLOAT_MIN;
        $data['min'] = PHP_FLOAT_MAX;

        // Iteramos por todas as transações para encontrar os valores máximo e mínimo.
        foreach ($data['transactions'] as $transaction) {
            $data['max'] = max($data['max'], $transaction['amount']);
            $data['min'] = min($data['min'], $transaction['amount']);
        }
    }
