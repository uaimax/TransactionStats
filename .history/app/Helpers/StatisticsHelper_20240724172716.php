...
class StatisticsHelper 
{
    ...
    public function cleanUpOldTransactions()
    {
        ...
        // Calcula o índice do minuto mais antigo permitido.
        $oldestMinuteIndex = intdiv($oldestTimestamp, 60) % $this->timeWindow;
        ...
        Log::info("Oldest minute index calculated: $oldestMinuteIndex");
    
        // Variáveis temporárias para verificação
        $initialCount = $this->count;
        $initialSum = $this->sum;
    
        // Remova todas as transações no índice do minuto mais antigo
        if (isset($this->transactions[$oldestMinuteIndex])) {
            foreach ($this->transactions[$oldestMinuteIndex] as $key => $transaction) {
                ...
                if ($transactionTimestamp < $oldestTimestamp) {
                    ...
                    unset($this->transactions[$oldestMinuteIndex][$key]);
                }
            }
    
            // Limpa o índice se necessário
            if (empty($this->transactions[$oldestMinuteIndex])) {
                unset($this->transactions[$oldestMinuteIndex]);
            } else {
                $this->transactions[$oldestMinuteIndex] = array_values($this->transactions[$oldestMinuteIndex]);
            }
    
            Log::info("Transactions at index $oldestMinuteIndex have been removed.");
        } else {
            Log::info("No transactions found at index $oldestMinuteIndex.");
        }
