...
class StatisticsHelper 
{
    ...
    public static function getStatistics() {
        ...
        Log::info('Hora atual', ['now' => $now->toDateTimeString()]);
        Log::info('Hora de expiração', ['expireTime' => $expireTime->toDateTimeString()]);
    ...
    {
        Log::info('Iniciando limpeza de transações antigas.', ['expireTime' => $expireTime]);
        ...
        while (!self::$transactions->isEmpty() && self::$transactions->bottom()['timestamp'] < $expireTime) {
            ...
            Log::info('Removendo transação mais antiga', ['oldestTransaction' => $oldestTransaction]);
    
            // Subtraímos o valor da transação removida
            self::$sum -= $oldestTransaction['amount'];
            Log::info('Subtraindo valor da soma total', ['amount' => $oldestTransaction['amount'], 'new_sum' => self::$sum]);
    
            // Subtraímos do contador de transações
            self::$count--;
            Log::info('Diminuindo contador de transações', ['new_count' => self::$count]);
            ...
            if (self::$count > 0) {
                ...
                Log::info('Atualizando min e max', ['new_min' => self::$min, 'new_max' => self::$max]);
            } else {
                // Se não houver mais transações, redefinimos o valor mínimo e máximo
                self::$min = null;
                self::$max = null;
                Log::info('Não há mais transações, redefinindo min e max');
            }
        }
    }
