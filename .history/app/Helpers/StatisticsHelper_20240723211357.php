...
class StatisticsHelper 
{
    ...
    public static function addTransaction($amount, $timestamp)
    {
        ...
        $expireTime = $now->copy()->addSeconds(60); // Quanto será 60 segundos no futuro

        // Verifica se o timestamp da transação é mais antigo que 60 segundos
        if ($timestamp->lt($now->subSeconds(60))) {
    ...
    public static function getStatistics() {
        ...
        $now = Carbon::now();
        $expireTime = $now->copy()->addSeconds(60); // Tempo de 60 segundos atrás
    ...
    public static function cleanUpOldTransactions($expireTime)
    {
        ...
        if (self::$transactions->isEmpty()) {
            ...
            // Log para verificar o tipo e valor do timestamp da transação mais antiga
            $oldestTransactionTimestamp = Carbon::parse(self::$transactions->bottom()['timestamp']);
            Log::info('Timestamp da transação mais antiga como Carbon', [
                'oldest_transaction_timestamp' => $oldestTransactionTimestamp->format('Y-m-d\TH:i:s.v\Z')
            ]);
    
            // Log para comparar os timestamps
            Log::info('Comparando timestamps', [
                'oldest_transaction_timestamp' => $oldestTransactionTimestamp->format('Y-m-d\TH:i:s.v\Z'),
                'expire_time' => $expireTime->format('Y-m-d\TH:i:s.v\Z'),
                'comparison_result' => $oldestTransactionTimestamp->lt($expireTime)
            ]);
        }
