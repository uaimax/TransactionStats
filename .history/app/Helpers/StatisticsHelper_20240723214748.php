...
class StatisticsHelper 
{
    ...
    public static function addTransaction($amount, $timestamp)
    {
        ...
        $expireTime = $now->subSeconds(60); // Definir 60 segundos atrás
    ...
    {
        Log::info('Iniciando limpeza de transações antigas.', ['expireTime' => $expireTime->format('Y-m-d\TH:i:s.v\Z')]);
        ...
        if (self::$transactions->isEmpty()) {
            ...
            Log::info('Timestamp da transação mais antiga como Carbon', ['oldest_transaction_timestamp' => Carbon::parse(self::$transactions->bottom()['timestamp'])->format('Y-m-d\TH:i:s.v\Z')]);
            Log::info('Comparando timestamps', [
                'oldest_transaction_timestamp' => Carbon::parse(self::$transactions->bottom()['timestamp'])->format('Y-m-d\TH:i:s.v\Z'),
                'expire_time' => $expireTime->format('Y-m-d\TH:i:s.v\Z'),
                'comparison_result' => Carbon::parse(self::$transactions->bottom()['timestamp'])->lt($expireTime)
            ]);
        }

        while (!self::$transactions->isEmpty() && Carbon::parse(self::$transactions->bottom()['timestamp'])->lt($expireTime)) {
            Log::info('Entrou no while');
